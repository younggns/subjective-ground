...
import torch
from custom_models import MoralGroundModel, SimpleLossCompute, NoamOpt ,BaselineClassifier
...
def run_eval(data, model, BATCH_SIZE, loss_compute):
    num_data = data.num_rows
    num_steps = int(num_data/BATCH_SIZE)
    total_f1 = 0
    total_loss = 0
...
def create_figure(model_path, NUM_EPOCHS, all_train_losses, all_train_accs, all_valid_losses, all_valid_accs):
    ...
    avg_train_losses_per_epoch = [sum(elem)/len(elem) for elem in all_train_losses]
    avg_train_accs_per_epoch = [sum(elem)/len(elem) for elem in all_train_accs]
    avg_valid_losses_per_epoch = [sum(elem)/len(elem) for elem in all_valid_losses]
    avg_valid_accs_per_epoch = [sum(elem)/len(elem) for elem in all_valid_accs]

    pickle.dump(avg_train_losses_per_epoch, open(model_path+'/avg_train_losses_per_epoch.pkl', 'wb'))
    pickle.dump(avg_train_accs_per_epoch, open(model_path+'/avg_train_accs_per_epoch.pkl', 'wb'))
    pickle.dump(avg_valid_losses_per_epoch, open(model_path+'/avg_valid_losses_per_epoch.pkl', 'wb'))
    pickle.dump(avg_valid_accs_per_epoch, open(model_path+'/avg_valid_accs_per_epoch.pkl', 'wb'))  

    plt.plot(range(NUM_EPOCHS), avg_train_losses_per_epoch, label = "train loss")
    plt.plot(range(NUM_EPOCHS), avg_train_accs_per_epoch, label = "train acc")
    plt.plot(range(NUM_EPOCHS), avg_valid_losses_per_epoch, label = "valid loss")
    plt.plot(range(NUM_EPOCHS), avg_valid_accs_per_epoch, label = "valid acc")
    plt.legend()
...
if __name__ == "__main__":
    ...
    all_authors = list(set(df.author))

    d_model = 768
    BATCH_SIZE = 16
    NUM_EPOCHS = 10
    all_tgts, all_preds = [], []
    model_name = 'baseline_noTrainer_5'
    for _author in all_authors:
        minidf = df[df.author==_author]

        train_data, eval_data, test_data = convert_df_to_datasets(minidf)

        model = BaselineClassifier(d_model, BATCH_SIZE, device).to(device)
        ...
        all_valid_losses, all_valid_accs = [], []
        ...
        best_epoch_model = BaselineClassifier(d_model, BATCH_SIZE, device).to(device).to(device)
        ...
        all_tgts += _author_tgts
        all_preds += _author_preds
    ...
    total_test_f1 = batch_f1(all_tgts, all_preds)
